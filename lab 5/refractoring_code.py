...
def draw_balloon(screen, colour, x, y, angle, size=80):
    ...
    Параметр colour - цвет воздушного шарика.
    Параметр angle - угол под которым воздушный шарик располагается к горизонту.
    Параметр size - размер воздушного шарика.
    '''
    x2, y2, x3, y3 = [
...
def draw_ice_cream(screen, x, y, angle, size):
    ...
    которая находится в самой нижней точке оси симметрии.
    Параметр angle - угол под которым мороженное располагается к горизонту.
    Параметр size - размер мороженного.
    '''
    x2, y2, x3, y3 = [
                      int(x - size * math.cos(math.radians(angle))),
                      int(y - size * math.sin(math.radians(angle))),
